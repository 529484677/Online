...
B = 20 # the Number of sub datasets,  N / n
N = n * B
p = 3
repetition = 500
...
t.fix = seq(0.02, 1.2, by = 0.1)  # Lambda(t.fix)
a0 = 0.9 # adjust cure rate, low with low, low cure = 0.9, high cure = 1.95
...
Getsummary1 = function(){
  ...
  rownames(our) = c('beta1.DF', 'beta2.DF','beta3.DF')
...
Get.data12 = function(n){
  #  study 1 and study 2: beta1*Z1 + beta2*Z2 + beta3*Z3
  ...
  data = data.frame(Y,delta,x1,x2,x3)
...
Get.data3 = function(n){
  #  study 3: beta1*Z1 + beta2*Z2 + beta3*Z3, Z3=1
  ...
  Y = pmin(T0, C)  # observed time
  delta = as.numeric(T0<=C)  # censoring indicator
  data = data.frame(Y,delta,x1,x2) # remove Z3 where Z3=1
...
CD.est = function(r){
  # get beta.est and beta.se using  online DF method  
  ...
  cureR = cenR = NULL
  for (b in 1:B){
    dat = Get.data12(n)  # generate data from study 1 with the model: g1 = exp(beta1*Z1+beta2*Z2+beta3*Z3)
    data = dat$data
    cureR[b] = dat$cureR
    cenR[b] = dat$cenR
  ...
  cureR = mean(cureR)
  cenR = mean(cenR)
  fit1 = list(est=beta.est, Sigma = beta.var, cenR=cenR, cureR=cureR)
  ...
  cureR = cenR = NULL
  for (b in 1:B){
    dat = Get.data12(n)  # generate data from study 2 with the model: g2 = exp(beta1*Z1+beta2*Z2+beta3*Z3)
    data = dat$data
    cureR[b] = dat$cureR
    cenR[b] = dat$cenR
    n = nrow(data); p = ncol(data)-2
  ...
  cureR = mean(cureR)
  cenR = mean(cenR)
  fit2 = list(est=beta.est, Sigma = beta.var, cenR=cenR, cureR=cureR)
  ...
  cureR = cenR = NULL
  for (b in 1:B){
    dat = Get.data3(n)  # generate data from study 3 with the model: g3 = exp(beta1*Z1+beta2*Z2+beta3*Z3), Z3 = 1
    data = dat$data
    cureR[b] = dat$cureR
    cenR[b] = dat$cenR
    n = nrow(data); p = ncol(data)-2
  ...
  cureR = mean(cureR)
  cenR = mean(cenR)
  fit3 = list(est=beta.est, Sigma = beta.var, cenR=cenR, cureR=cureR)
  
  
  # the online DF method 
  fit = list(fit1, fit2, fit3)
  K=3
  p = 3  # the number of all parameters, beta1, beta2, beta3
  pi= c(3,3,2)  # (beta1, beta2, beta3) in study 1, (beta1, beta2, beta3) in study 2, (beta1, beta2) in study 3 
  est = Sigma = B = list()
  for(k in 1:K){
    est[[k]] = fit[[k]]$est
    Sigma[[k]] = fit[[k]]$Sigma
  ...
  i = 1
  B[[i]] = diag(1, nrow = pi[i],  ncol = p)  # dim is 3*3
  i = 2
  B[[i]] = diag(1, nrow = pi[i],  ncol = p)  # dim is 3*3
  i = 3
  B[[i]] = matrix(c(1,0,0
                    ,0,1,0), nrow = 2, ncol = 3, byrow = TRUE)  # dim is 2*3
  
  
  CD = function(est, Sigma, B, p=3, K=3){
    # est: beta.est, dim is pi*1 in a list
    # Simga: variace matrix of beta.est, dim is pi*pi in a list
    # B: map function, dim is pi*p* in a list
    # p: dim of beta
    # pi: the number of different models from hetergeous Z
    # K: the number of studies
    Right = rep(0,p)
    Left = matrix(0, p, p)
    for (k in 1:K) {
      Bi = B[[k]]  # dim is pi*p
      Sigmai = Sigma[[k]]  # dim is pi*pi
      esti = est[[k]]  # dim is pi*1
      Left = Left + t(Bi) %*% solve(Sigmai) %*% Bi  # dim is p*p, Bi'Sigmai^-1Bi
      Right = Right + t(Bi) %*% solve(Sigmai) %*% esti   # dim is p*1, Bi'Sigmai^-1est
    }
    var = solve(Left)
    est = var  %*% Right
  ...
  CD.result = CD(est, Sigma, B, p = 3, K = 3)
  beta.est = CD.result$est
  beta.se = sqrt(diag(CD.result$var))
  CD.result = cbind(beta.est, beta.se)
  rownames(CD.result) = paste('Beta', 1:p, seq='')
  colnames(CD.result) = c('Est', 'SE')
  
  cenR = mean(c(fit1$cenR, fit2$cenR, fit3$cenR))
  cureR = mean(c(fit1$cureR, fit2$cureR, fit3$cureR))
  
  
  return(list(beta.est=beta.est, beta.se=beta.se, cenR=cenR, cureR=cureR))
}
...
MyRsult = foreach(r = 1:repetition, .combine = rbind, .multicombine = TRUE, .packages = pklist, .export=varlist, .inorder=FALSE)  %dopar%  CD.est(r)
